export const ru = {
  app_title: 'MIERU PANEL',
  nav_dashboard: 'Дашборд',
  nav_users: 'Пользователи',
  nav_stats: 'Статистика',
  nav_server: 'Сервер',
  nav_settings: 'Настройки',
  nav_logs: 'Логи',
  nav_logout: 'Выйти',
  users_add: 'Добавить',
  users_retry: 'Повторить',
  users_empty: 'Пользователей пока нет',
  users_search_no_match: 'Ничего не найдено',
  users_search_clear: 'Очистить фильтр',
  users_loading: 'Загрузка пользователей',
  app_loading: 'Загрузка…',
  users_col_name: 'Пользователь',
  users_col_quota_day: 'Квота день',
  users_col_quota_month: 'Квота месяц',
  users_col_actions: 'Действия',
  users_action_sub: 'Подписка',
  users_action_regen: 'Обновить пароль',
  users_action_delete: 'Удалить',
  login_title: 'Вход в панель',
  login_username: 'Логин',
  login_password: 'Пароль',
  login_btn: 'Войти',
  login_error: 'Неверный логин или пароль',
  admin_settings_title: 'Учётная запись администратора',
  admin_settings_hint: 'Нужны текущий пароль и новые логин с паролем.',
  admin_settings_current_password: 'Текущий пароль',
  admin_settings_new_username: 'Новый логин',
  admin_settings_new_password: 'Новый пароль',
  admin_settings_submit: 'Сохранить',
  admin_username_error: 'Логин: a-z, A-Z, 0-9, _, -, длина 2-32',
  admin_password_too_short: 'Пароль: минимум 5 символов',
  toast_admin_credentials_saved: 'Логин и пароль администратора обновлены',
  dashboard_title: 'Операционный центр',
  dashboard_hint: 'Итерация 1: базовая архитектура React готова.',
  tab_users: 'Пользователи',
  tab_stats: 'Статистика',
  tab_server: 'Сервер',
  tab_logs: 'Логи',
  stat_total_users: 'Всего пользователей',
  stat_total_traffic_today: 'Трафик сегодня',
  stat_server_status: 'Состояние сервера',
  server_running: 'RUNNING',
  server_idle: 'IDLE',
  server_offline: 'OFFLINE',
  server_start: 'Запустить',
  server_stop: 'Остановить',
  server_section_title: 'Управление mita',
  server_status_title: 'Текущий статус',
  stats_chart_title: 'Суточный трафик по пользователям',
  stats_table_title: 'Разбивка по пользователям',
  stats_day: 'Сегодня',
  stats_month: 'Месяц',
  stats_chart_hint:
    'Столбики — суточный расход (МБ) из данных панели по каждому пользователю. Live-метрики mita сюда не подтягиваются.',
  stats_chart_empty: 'Нет пользователей для графика.',
  stats_chart_axis_mb: 'МБ',
  sub_url_label: 'URL подписки',
  sub_copy: 'Скопировать',
  sub_qr_hint: 'Вставьте URL в Karing → Profiles → + → URL',
  sub_new_password: 'Новый пароль',
  modal_add_title: 'Новый пользователь',
  modal_name: 'Логин',
  modal_password: 'Пароль',
  modal_quota_day: 'Лимит в сутки (МБ)',
  modal_quota_month: 'Лимит в месяц (МБ)',
  modal_save: 'Создать',
  modal_cancel: 'Отмена',
  modal_name_error: 'Логин: a-z, A-Z, 0-9, _, -, длина 2-32',
  modal_inline_error: 'Не удалось создать пользователя',
  confirm_delete_title: 'Удалить пользователя',
  confirm_delete_message: 'Удалить пользователя {{name}}?',
  confirm_yes: 'Удалить',
  confirm_no: 'Отмена',
  toast_user_added: 'Пользователь создан',
  toast_user_deleted: 'Пользователь удален',
  toast_password_regenerated: 'Пароль обновлен',
  toast_copied: 'Скопировано',
  toast_server_auto_started: 'Сервер mita запущен автоматически',
  empty_state_title: 'Добавь первого пользователя',
  empty_state_hint:
    'Сразу после создания пользователя сервер mita запустится автоматически. Подписка и QR-код появятся в карточке пользователя.',
  toast_error: 'Ошибка сервера',
  quota_unlimited: 'Без лимита',
  unit_mb: 'МБ',
  unit_gb: 'ГБ',
  theme_midnight: 'Midnight',
  theme_sakura: 'Sakura',
  theme_ghost: 'Ghost',
  theme_switcher_aria: 'Оформление',
  lang_switcher_aria: 'Язык интерфейса',
  server_settings_title: 'Параметры сервера',
  server_settings_hint:
    'Эти параметры используются в подписке для каждого пользователя и в конфиге mita. Изменения применяются к mita сразу.',
  server_settings_ip: 'Публичный IP сервера',
  server_settings_port: 'Порт по умолчанию',
  server_settings_range: 'Диапазон портов (например 2012-2022)',
  server_settings_save: 'Сохранить и применить',
  server_settings_saved: 'Настройки сервера сохранены',
  server_settings_warning: 'Сохранено, но mita не подхватил: {{warning}}',
  server_settings_invalid_range: 'Формат диапазона: 2012-2022',
  logs_title: 'Логи панели',
  logs_hint:
    'Все события панели и запросы к mita CLI. Также пишутся в stdout, видны в `docker logs mieru-panel`.',
  logs_empty: 'Пока пусто.',
  logs_pause: 'Пауза',
  logs_resume: 'Продолжить',
  logs_clear: 'Очистить экран',
  logs_filter_level: 'Уровень',
  logs_filter_all: 'все',
  logs_mita_title: 'Логи mita',
  logs_mita_hint:
    'Хвост stdout демона mita из общего лог-файла /var/log/mita/mita.log. Те же строки доступны через `docker compose logs mita`.',
  logs_mita_unavailable:
    'Лог-файл mita ещё не создан. Подожди, пока mita стартует, или проверь `docker compose logs mita`.',
  logs_mita_refresh: 'Обновить',
  logs_mita_truncated_hint:
    'Вывод обрезан. Для полного лога используйте `docker compose logs mita`.',
  user_show_config: 'Показать sing-box JSON',
  user_hide_config: 'Скрыть JSON',
  user_copy_config: 'Скопировать JSON',
  // header
  header_status_aria: 'Статус mita-прокси',
  // theme
  theme_daylight: 'Daylight',
  // user table
  users_col_lastactive: 'Активность',
  users_search_placeholder: 'Поиск пользователя…',
  users_export_subs: 'Экспорт всех подписок',
  users_dblclick_regen_confirm: 'Перевыпустить пароль для {{name}}?',
  users_dblclick_regen_hint: 'Двойной клик — перевыпустить пароль',
  users_edit_expiry_hint: 'Изменить срок действия',
  users_expired: 'просрочен',
  toast_user_updated: 'Пользователь обновлён',
  // quota presets
  modal_quota_preset: 'Пресет квоты',
  quota_preset_unlimited: 'Без лимита',
  quota_preset_1gb_day: '1 ГБ/день · 30 ГБ/мес',
  quota_preset_30gb_month: '30 ГБ/мес',
  quota_preset_500mb_day: '500 МБ/день · 10 ГБ/мес',
  // expiry presets
  modal_expiry: 'Срок действия',
  expiry_never: 'Бессрочно',
  expiry_7d: '7 дней',
  expiry_30d: '30 дней',
  expiry_90d: '90 дней',
  expiry_365d: '1 год',
  // connections panel
  connections_title: 'Активные подключения',
  connections_hint:
    'Сессии, которые сейчас обрабатывает mita. Обновляется каждые 10 секунд.',
  connections_count_suffix: 'сессий',
  connections_empty: 'Активных подключений нет.',
  connections_unavailable: 'Подключения недоступны: mita ещё не запущен.',
  connections_col_remote: 'Клиент',
  connections_col_protocol: 'Протокол',
  connections_col_state: 'Состояние',
  connections_col_recvq: 'Recv Q+Buf',
  connections_col_sendq: 'Send Q+Buf',
  connections_col_lastrecv: 'Последний приём',
  // advanced settings
  advanced_title: 'Дополнительные параметры mita',
  advanced_hint:
    'Уровень логирования, MTU и режим мультиплексирования. Применяются на лету через apply config + reload.',
  advanced_log_level: 'Уровень логов',
  advanced_mtu: 'MTU (1280–1500)',
  advanced_multiplexing: 'Мультиплексирование',
  advanced_saved: 'Параметры применены',
  advanced_mtu_error: 'MTU должен быть в диапазоне 1280–1500',
  // backup
  backup_title: 'Резервная копия',
  backup_hint:
    'Скачай текущий config.json или восстанови панель из ранее сделанной копии. Порт и сессионный ключ остаются текущими, чтобы не разлогинило.',
  backup_download: 'Скачать backup.json',
  backup_restore: 'Загрузить backup',
  backup_restore_confirm:
    'Восстановить конфиг из файла? Текущие пользователи и параметры будут заменены.',
  backup_restore_ok: 'Конфиг восстановлен, mita пересинхронизирован.',
  // misc
  loading: 'Загрузка…',
  saving: 'Сохранение…',
  sub_quota_edit_title: 'Изменить квоты',
  sub_quota_save: 'Применить квоты',
  // themes
  theme_solar: 'Solar',
  theme_cyber: 'Cyber',
  theme_auto: 'Auto',
  // device limit
  modal_device_limit: 'Лимит устройств',
  modal_device_limit_hint:
    'Сервер запоминает уникальные User-Agent при заходе по подписке. Если устройств больше — клиенту 403.',
  device_preset_unlimited: 'Без лимита',
  device_preset_one: '1 устройство',
  device_preset_two: '2 устройства',
  device_preset_three: '3 устройства',
  device_preset_five: '5 устройств',
  users_devices_pill_hint: 'Количество запомненных устройств / лимит',
  sub_devices_title: 'Привязанные устройства',
  sub_devices_empty: 'Пока никто не загружал подписку.',
  sub_devices_unknown_ua: 'Неизвестный клиент',
  sub_devices_remove: 'Отвязать',
  sub_devices_reset_all: 'Сбросить все',
  sub_devices_reset_confirm: 'Отвязать все устройства? Они смогут заново забрать подписку.',
  sub_devices_reset_ok: 'Список устройств очищен',
  sub_devices_remove_ok: 'Устройство отвязано',
  // bulk
  users_select_all: 'Выбрать всех',
  users_select_one: 'Выбрать {{name}}',
  users_bulk_selected: 'Выбрано: {{count}}',
  users_bulk_delete: 'Удалить выбранных',
  users_bulk_confirm: 'Удалить {{count}} пользователей?',
  users_bulk_deleted: 'Удалено пользователей: {{count}}',
  // audit
  audit_title: 'Аудит-журнал',
  audit_hint:
    'Все security-significant события: входы, CRUD пользователей, выдача подписок, смена настроек. Файл append-only.',
  audit_empty: 'Аудит-журнал пока пуст.',
  audit_load_more: 'Загрузить ещё {{count}}',
  // sub security
  sub_security_title: 'Защита подписки',
  sub_security_hint:
    'Привязка /sub/<token> к конкретным клиентам. Так невозможно скачать профиль через curl, а смена User-Agent в настройках клиента не помогает обойти лимит устройств — фингерпринт берётся из X-HWID.',
  sub_security_allowed_label: 'Разрешённые клиенты (User-Agent, по одному на строку):',
  sub_security_allowed_placeholder: 'Karing\nsing-box\nmihomo',
  sub_security_allowed_help:
    'Подстроки сравниваются без регистра. Пустой список = принимать любой клиент.',
  sub_security_preset_official: 'Только официальные клиенты',
  sub_security_preset_karing: 'Только Karing',
  sub_security_preset_any: 'Любой клиент',
  sub_security_require_hwid: 'Требовать заголовок X-HWID',
  sub_security_require_hwid_hint:
    'Karing/sing-box шлют X-HWID автоматически. Включение блокирует «голые» парсеры и форки без HWID.',
  sub_devices_hwid: 'Hardware ID клиента',
  sidebar_collapse: 'Свернуть боковую панель',
  sidebar_expand: 'Развернуть боковую панель',
  topbar_breadcrumb_home: 'mieru',
  topbar_breadcrumb_dashboard: 'Дашборд',
  topbar_breadcrumb_users: 'Пользователи',
  topbar_breadcrumb_server: 'Сервер',
  topbar_breadcrumb_settings: 'Настройки',
  topbar_breadcrumb_logs: 'Логи',
  topbar_admin_menu: 'Меню администратора',
  topbar_open_logs: 'Открыть логи',
  mobile_menu_open: 'Открыть меню',
  mobile_menu_close: 'Закрыть меню',
  status_running: 'RUNNING',
  status_stopped: 'STOPPED',
  status_warning: 'WARNING',
  status_unknown: 'UNKNOWN',
  'kpi.users': 'Пользователи',
  'kpi.users_active_suffix': 'активных',
  'kpi.today_traffic': 'Трафик сегодня',
  'kpi.today_traffic_suffix': 'у {{count}} пользователей',
  'kpi.connections': 'Активные подключения',
  'kpi.connections_no_active': 'нет активных сессий',
  'kpi.connections_checking': 'Проверка…',
  'kpi.connections_sub_active': '{{count}} активных сессий',
  'kpi.connections_data_unavailable': 'Временно недоступно',
  'kpi.connections_server_stopped': 'сервер остановлен',
  'kpi.server_status': 'Статус сервера',
  'kpi.status_since': 'с:',
  'top_users.today_title': 'Сегодня',
  'top_users.month_title': 'Месяц',
  'top_users.empty_today': 'Нет трафика сегодня',
  'top_users.empty_month': 'Нет трафика за месяц',
  'top_users.show_all': 'Показать все',
  'auth.2fa.section_title': 'Двухфакторная аутентификация',
  'auth.2fa.section_hint': 'Одноразовые коды из приложения (TOTP). Резервные коды храните офлайн.',
  'auth.2fa.loading': 'Загрузка…',
  'auth.2fa.not_configured': '2FA не настроена',
  'auth.2fa.enable_cta': 'Включить',
  'auth.2fa.active_pill': 'Активна',
  'auth.2fa.backup_remaining': 'Резервных кодов: {{count}}',
  'auth.2fa.regenerate': 'Регенерировать резервные коды',
  'auth.2fa.disable': 'Отключить 2FA',
  'auth.2fa.login_step_title': 'Код входа',
  'auth.2fa.login_step_hint': 'Введите код из приложения‑аутентификатора.',
  'auth.2fa.totp_placeholder': '000000',
  'auth.2fa.use_backup_link': 'Использовать резервный код',
  'auth.2fa.use_totp_link': 'Использовать код из приложения',
  'auth.2fa.backup_placeholder': 'XXXX-XXXX',
  'auth.2fa.backup_code_label': 'Резервный код',
  'auth.2fa.code_invalid': 'Неверный код',
  'auth.2fa.locked_minutes': 'Слишком много попыток, попробуйте через {{minutes}} мин.',
  'auth.2fa.cancel': 'Назад',
  'auth.2fa.setup_intro_title': 'Включение 2FA',
  'auth.2fa.setup_intro_body':
    'Подключите приложение (Google Authenticator, Aegis и т.п.) и сохраните резервные коды в безопасном месте.',
  'auth.2fa.setup_begin': 'Начать',
  'auth.2fa.setup_scan_title': 'Отсканируйте QR или введите секрет',
  'auth.2fa.setup_manual_secret': 'Секрет',
  'auth.2fa.setup_verify_code': 'Код из приложения',
  'auth.2fa.setup_submit': 'Проверить и продолжить',
  'auth.2fa.setup_backups_title': 'Сохраните резервные коды',
  'auth.2fa.setup_backups_hint': 'Каждый код одноразовый. Мы покажем их только сейчас.',
  'auth.2fa.setup_download_txt': 'Скачать .txt',
  'auth.2fa.setup_copy_all': 'Скопировать всё',
  'auth.2fa.setup_next': 'Далее',
  'auth.2fa.setup_ack_label': 'Я сохранил резервные коды',
  'auth.2fa.setup_finish': 'Завершить',
  'auth.2fa.setup_cancel': 'Отмена',
  'auth.2fa.modal_disable_title': 'Отключить 2FA',
  'auth.2fa.modal_regenerate_title': 'Новые резервные коды',
  'auth.2fa.modal_password': 'Пароль',
  'auth.2fa.modal_code': 'Код из приложения',
  'auth.2fa.modal_confirm': 'Подтвердить',
  'auth.2fa.done': 'Готово',
  'auth.2fa.toast_enabled': 'Двухфакторная аутентификация включена',
  'auth.2fa.toast_disabled': 'Двухфакторная аутентификация отключена',
  'auth.2fa.toast_regenerated': 'Резервные коды обновлены',
  saved: 'Сохранено',
  save: 'Сохранить',
  action_refresh: 'Обновить',
  'dashboard.chart_warming_up':
    'Идёт сбор метрик, графики появятся через несколько минут.',
  'dashboard.kpi_title': 'Общая сводка',
  'dashboard.kpi_established': 'Открытые сессии',
  'dashboard.kpi_max_conn': 'Пиковые подключения',
  'dashboard.kpi_errors_hour': 'Ошибки за час',
  'dashboard.kpi_panel_uptime': 'Аптайм панели',
  'dashboard.kpi_download_total': 'Скачано всего',
  'dashboard.kpi_upload_total': 'Загружено всего',
  'dashboard.chart_bandwidth': 'Пропускная способность',
  'dashboard.chart_errors': 'Ошибки',
  'dashboard.chart_rx': 'Входящий (МБ)',
  'dashboard.chart_tx': 'Исходящий (МБ)',
  'dashboard.resources_title': 'Ресурсы хоста',
  'dashboard.resources_hint': 'Живые замеры с хоста панели.',
  'dashboard.mem': 'Память',
  'dashboard.net_rx': 'Net RX',
  'dashboard.net_tx': 'Net TX',
  'dashboard.top_users_section': 'Топ по трафику',
  'dashboard.mita_internals_title': 'Метрики mita',
  'dashboard.mita_internals_hint': 'Значения из daemon metrics JSON.',
  'dashboard.mita_waiting': 'Ожидание данных…',
  'dashboard.recent_audit_title': 'Последние события аудита',
  'dashboard.recent_audit_hint': 'Недавние события, значимые для безопасности.',
  'dashboard.audit_empty': 'Событий пока нет.',
  'dashboard.metrics_poll_hint': 'Метрики дашборда',
  'dashboard.daemon_aria_start': 'Запустить демон',
  'dashboard.daemon_aria_restart': 'Перезапустить демон',
  'dashboard.daemon_aria_stop': 'Остановить демон',
  'dashboard.daemon_port_label': 'порт',
  'logs_drawer.search_placeholder': 'Поиск ⌘K',
  'logs_drawer.autoscroll_aria': 'Автопрокрутка',
  'logs_drawer.clear_aria': 'Очистить экран',
  'logs_drawer.download_aria': 'Скачать отфильтрованные логи',
} as const
